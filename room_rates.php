...
if ($rows !== null) {
    // Check if 'STATUS' key exists in $rows array
    if (array_key_exists('STATUS', $rows)) {
        $status = $rows['STATUS'];
    } else {
        // Handle the case when 'STATUS' key doesn't exist
        $status = null; // Or set a default value
    }
} else {
    // Handle the case when $rows is null
    $status = null; // Or set a default value
}
